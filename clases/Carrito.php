...
class Carrito
{
    ...
    function __construct($idUnico, $dniCliente = null, $idProducto = null, $cantidad = null)
    {
    ...
    function insertar($con)
    {
        $time = time();
        try {
            $result = $con->prepare('INSERT INTO carrito values(:idUnico, :dni, :idProducto, :cantidad, :time_)');
            $result->bindParam(':idUnico', $this->idUnico);
    ...
    static function deleteAll($con, $idUnico) {
        $result = $con->prepare('DELETE FROM carrito WHERE idUnico = :idUnico');
        $result->bindParam(':idUnico', $idUnico);
        $result->execute();
    }
}
